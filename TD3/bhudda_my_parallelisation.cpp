...
int main()
{
    ...
    std::vector<unsigned> buddha1, buddha2, buddha3;
    //const unsigned long l1 = 20000, l2 = 100000, l3 = 1000000;
    const unsigned long l1 = 2000, l2 = 10000, l3 = 10000;
    #pragma omp parallel num_threads(3) default (none) private ( start, end, elapsed_seconds) shared (width, height,l1, l2, l3, std::cerr, std::cout, buddha1, buddha2, buddha3)
    {
        ...
        if (rank == 0)
        {
            ...
            std::cout << "Temps calcul Bhudda 1 : " << elapsed_seconds.count() << "; 1:" << width
                      << std::endl;
        }
        if (rank == 1)
        {
            ...
            elapsed_seconds = end-start;
            std::cout << "Temps calcul Bhudda 2 : " << elapsed_seconds.count() << "; 2:" << height
                      << std::endl;
        }
        if (rank == 2)
        {
            ...
            elapsed_seconds = end-start;
            std::cout << "Temps calcul Bhudda 3 : " << elapsed_seconds.count() << "; 3:" << buddha3.size ()
                      << std::endl;
        }
    ...
    for ( unsigned i = 0; i < height; ++i ) {
        for (unsigned j = 0; j < width; ++j ) {
            ...
            //std::cout << ind << "; 1:" << buddha1.size ()<< "; 2:" << buddha2.size () << "; 3:"<< buddha3.size () << std::endl;
            b1 += buddha1[ind];
            b2 += buddha2[ind];
            b3 += buddha3[ind];
        }
